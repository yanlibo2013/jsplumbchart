...
var __vue_normalize__ = _interopDefault(require('C:workpackagejsplumbchart
ode_modules_rollup-plugin-vue@5.1.4@rollup-plugin-vueuntime
ormalize.js'));
var __vue_create_injector__ = _interopDefault(require('C:workpackagejsplumbchart
ode_modules_rollup-plugin-vue@5.1.4@rollup-plugin-vueuntimerowser.js'));
...
var script = {
  ...
  methods: {
    ...
    delAllselected: function delAllselected(data) {
      this.stepData = lodash.filter(data, function (item) {
    ...
    selectCurrentStep: function selectCurrentStep(val) {
      ...
      this.stepData = lodash.map(this.stepData, function (item) {
        if (val.id == item.id) {
    ...
    drawJsplumbChart: function drawJsplumbChart(data, connectCallback) {
      ...
      flowchart.addEndpointToNode(data.jsplumbInstance, data.self, data.flowData, function (val) {
        _this4.stepData = lodash.map(_this4.stepData, function (item) {
          if (item.id == val.id) {
            return Object.assign({}, item, {
              x: val.x,
              y: val.y
    ...
    delNode: function delNode(val) {
      this.stepData = lodash.filter(lodash.cloneDeep(this.stepData), function (item) {
        return item.id != val;
      });
    ...
    getLinksData: function getLinksData() {
      this.links = flowchart.filterLinkData(lodash.map(this.jsplumbInstance.getAllConnections(), function (item) {
        return {
          name: item.id,
      ...
      }), lodash); //console.log("  getLinksData() {", this.links);
    },
    ...
    setLineSplit: function setLineSplit(step) {
      ...
      if (step.type == "multioutput") {
        var outputConfigurations = lodash.toArray(step.outputConfigurations); // console.log("step.outputConfigurations", step.outputConfigurations);
